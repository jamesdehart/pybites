"""
Complete the two functions: years_ago and convert_eu_to_us_date following the
  instructions in their docstrings.

This is the defintion and difference between the two:

strptime: parse (convert) string to datetime object.
strftime: create formatted string for given time/date/datetime object according
to specified format.
"""
...
def years_ago(date):
    ...
    arg_year = datetime.strptime(date, "%d %b, %Y").year
    return THIS_YEAR - arg_year
...
def convert_eu_to_us_date(date):
    ...
    euro = datetime.strptime(date, "%d/%m/%Y")
    return euro.strftime("%m/%d/%Y")
